...
#_setupRowNumber(){if(typeof this.#_props.show_row_number==='boolean'&&this.#_props.show_row_number){const hasIndexColumn=this.#_props.columns.find((column)=>column.data=='rowNumber');if(!hasIndexColumn){let maxRowSpan=1;$(`${this.#_id} thead th`).each((index,th)=>{const thRowSpan=$(th).attr('rowspan');if(thRowSpan>maxRowSpan){maxRowSpan=thRowSpan}});const indexColumn=`<th rowspan="${maxRowSpan}" class="column-row-number">#</th>`;$(`${this.#_id} thead tr:first-child`).prepend($(indexColumn));if($(`${this.#_id} tfoot tr:first-child`).length>0){$(`${this.#_id} tfoot tr:first-child`).prepend($(indexColumn))}
this.#_props.columns.unshift({data:'rowNumber',searchable:!1,orderable:!1,sortable:!1,className:'column-row-number',width:30,})}}}
#_setupCheckboxColumn(){if(typeof this.#_props.show_checkbox==='boolean'&&this.#_props.show_checkbox){const hasCheckboxColumn=this.#_props.columns.find((column)=>column.data=='checkbox');if(!hasCheckboxColumn){let maxRowSpan=1;$(`${this.#_id} thead th`).each((index,th)=>{const thRowSpan=$(th).attr('rowspan');if(thRowSpan>maxRowSpan){maxRowSpan=thRowSpan}});const checkbox_header_class=this.#_default_checkbox_column_header_class[0]=='.'?this.#_default_checkbox_column_header_class.slice(1):this.#_default_checkbox_column_header_class;const checkboxColumn=`<th rowspan="${maxRowSpan}" class="${checkbox_header_class} dt-center">
...
const showRowNumber=this.#_props.show_row_number;const showRowReorder=this.#_props.show_row_reorder;let default_select_prop={style:'multiple',};if(showRowNumber&&showRowReorder){default_select_prop.selector='td:nth-child(3) input[type="checkbox"]'}else{if(showRowNumber){default_select_prop.selector='td:nth-child(2) input[type="checkbox"]'}else{default_select_prop.selector='td:first-child input[type="checkbox"]'}}
...
#_setupRowReorderColumn(){if(typeof this.#_props.show_row_reorder==='boolean'&&this.#_props.show_row_reorder){const hasRowReorderColumn=this.#_props.columns.find((column)=>column.data=='rowReorder');if(!hasRowReorderColumn){let maxRowSpan=1;$(`${this.#_id} thead th`).each((index,th)=>{const thRowSpan=$(th).attr('rowspan');if(thRowSpan>maxRowSpan){maxRowSpan=thRowSpan}});const rowReorder_header_class=this.#_default_rowreorder_column_header_class[0]=='.'?this.#_default_rowreorder_column_header_class.slice(1):this.#_default_rowreorder_column_header_class;const rowReorderColumn=`<th rowspan="${maxRowSpan}" class="${rowReorder_header_class} dt-center"></th>`;const rowReorderColumnElement=$(rowReorderColumn);$(`${this.#_id} thead tr:first-child`).prepend(rowReorderColumnElement);const rowReorder_class=this.#_default_rowreorder_column_class[0]=='.'?this.#_default_rowreorder_column_class.slice(1):this.#_default_rowreorder_column_class;this.#_props.columns.unshift({data:'rowReorder',searchable:!1,orderable:!0,sortable:!1,className:`dt-center reorder ${rowReorder_class}`,width:20,render:function(data,type,row,meta){return `≡`}});const rowReorder=this.#_props.rowReorder;let default_rowReorder_prop={selector:'td.reorder',update:!1,};if(typeof rowReorder==='boolean'){this.#_props.rowReorder=default_rowReorder_prop}else{this.#_props.rowReorder=_.merge(default_rowReorder_prop,this.#_props.rowReorder)}}}}
