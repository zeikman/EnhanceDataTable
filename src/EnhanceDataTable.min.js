...
{#_debug=!1;#_datatable;#_id;#_checkbox_header_triggered;#_default_thead;#_default_checkbox_column_header_class='.column-checkbox-header';#_default_checkbox_column_class='.column-checkbox';#_current_page=0;#_props={column_hide_in_card:[],three_states_sort:!0,show_row_number:!0,show_checkbox:!1,checked_visible_only:!1,enable_checkbox_event:!1,search:{return:!0,},language:{search:'',searchPlaceholder:'ENTER Search; ESC Clear',},};#_initDataTable(){const proceed_init=this.#_setupCheckboxClasses();if(!proceed_init){return proceed_init}
if(this.#_debug){console.error(`init : ${this.#_id}`)}
this.#_retainDefaultTheadStructure();this.#_setupRowCallback();this.#_setupDrawCallback();this.#_setupInitComplete();this.#_setupCheckboxColumn();this.#_setupRowNumber();$(this.#_id).data('view-status','table');if(this.#_props.hasOwnProperty('column_hide_in_card')){$(this.#_id).data('card-hide-col',this.#_props.column_hide_in_card)}
const datatable_id=this.#_id;if($(datatable_id).length>0){const props=this.#_props;const wrapper=`${datatable_id}_wrapper`;this.#_datatable=$(datatable_id).DataTable(props);this.dataTable=this.#_datatable;$(wrapper).addClass('table-view');return!0}else{console.error(`[EnhanceDataTable] Error: DOM element '${datatable_id}' not found !`);window.alert(`[EnhanceDataTable] Error: DOM element '${datatable_id}' not found !`);return!1}}
...
#_setupCheckboxColumn(){if(this.#_props.show_checkbox){const hasCheckboxColumn=this.#_props.columns.find((column)=>column.data=='checkbox');if(!hasCheckboxColumn){const checkboxColumn=`<th class="column-checkbox-header dt-center">
          <input type="checkbox" class="form-check-input" />
        </th>`;const checkboxColumnElement=$(checkboxColumn);$(`${this.#_id} thead tr:first-child`).prepend(checkboxColumnElement);this.#_props.columns.unshift({data:'checkbox',searchable:!1,orderable:!1,sortable:!1,className:'dt-center',width:21,render:function(data,type,row,meta){return `<input type="checkbox" class="form-check-input column-checkbox" />`}})}
const showRowNumber=this.#_props.show_row_number;let default_select_prop={style:'multiple',};if(showRowNumber){default_select_prop.selector='td:nth-child(2) input[type="checkbox"]'}else{default_select_prop.selector='td:first-child input[type="checkbox"]'}
...
this.#_props=_.merge(this.#_props,args);const init_success=this.#_initDataTable();if(init_success){this.#_renderRowNumberEvent();this.#_columnVisibilityEvent();this.#_selectDeselectEvent()}}
...
return[]}
getSelectedRowIds(condition='.selected'){const ids=[];const selectedData=this.#_datatable.rows(condition).data();for(let index=0;index<selectedData.length;index++){const data=selectedData[index];if(Array.isArray(data)){data.forEach(element=>{if(element.indexOf('type="checkbox"')>-1){const checkboxElement=$(element);const id=checkboxElement.data('id');ids.push(id)}})}else{ids.push(data.id)}}
return ids}
getVisibleSelectedRowIds(){return this.getSelectedRowIds('.selected:visible')}
getSelectedRowDatas(condition='.selected'){const datas=[];const selectedData=this.#_datatable.rows(condition).data();for(let index=0;index<selectedData.length;index++){datas.push(selectedData[index])}
return datas}
getVisibleSelectedRowDatas(){return this.getSelectedRowDatas('.selected:visible')}
